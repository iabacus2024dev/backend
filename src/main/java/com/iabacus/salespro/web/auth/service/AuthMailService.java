...
public class AuthMailService {
    ...
    public void sendInitializePasswordLink(String to, String token) {
        String link = String.format("%s/auths/initialize?token=%s", baseUrl, token);
        ...
        String body = String.format(
            "<div style='font-family: Arial, sans-serif; padding: 20px; max-width: 480px; margin: auto;'>"
                ...
                + "<a href='%s' style='display: inline-block; background-color: #EB6129; color: #fff; padding: 12px 24px; font-size: 16px; text-decoration: none; border-radius: 6px; margin: 20px 0;'>비밀번호 초기화</a>"
                + "<p style='font-size: 10px; color: #777;'>버튼이 작동하지 않으면 아래 링크를 복사하여 브라우저에 붙여넣으세요:</p>"
                + "<p style='font-size: 10px; color: #EB6129; word-break: break-word;'>%s</p>"
